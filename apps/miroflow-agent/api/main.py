...
class PlanRequest(BaseModel):
    """Request for /v1/api/plan endpoint"""
    query: str
    history: Optional[List[Message]] = None


class ExecuteRequest(BaseModel):
    """Request for /v1/api/execute endpoint"""
    plan: List[Message]
...
async def stream_generator(
    ...
    """
    Generate streaming responses in the new format.
    ...
    Args:
        ...
        is_planning: Whether this is planning phase (True) or execution phase (False)
    ...
    async def consume_stream():
        ...
        try:
            while True:
                ...
                # Transform events to new format
                if event_type == "tool_call":
                    ...
                    # Messages are assistant responses
                    ...
                    if is_planning:
                        ...
                        output = {
                            "type": "plan",
                            "step": current_step,
                            "delta": f"Starting agent: {agent_name}"
                        }
                        yield json.dumps(output, ensure_ascii=False) + "\n"
            ...
            error_output = {
                "type": "answer" if not is_planning else "plan",
                "step": current_step if current_step > 0 else 1,
                "delta": f"Error: {str(e)}"
            }
...
def main(cfg: DictConfig) -> None:
    ...
    Allows overriding configuration from command line:
        python -m api.main llm=qwen-3 llm.api_key=xxxxx llm.base_url=http://localhost:8000/v1
    """
    global _default_cfg
